...
EXTRA_PROVE_ARGS ?=
# PROVE_COMMAND: If you want to use a different command than prove
PROVE_COMMAND ?= prove
ifeq ($(TESTS),)
...
test-unit-and-integration: node_modules
	...
	RETRY=${RETRY} HOOK=./tools/delete-coverdb-folder timeout -s SIGINT -k 5 -v ${TIMEOUT_RETRIES} tools/retry ${PROVE_COMMAND} ${PROVE_LIB_ARGS} ${PROVE_ARGS}
