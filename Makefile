...
.PHONY: check-standard
check-standard:
	@which standard >/dev/null 2>&1 || (echo "Command 'standard' not found, use 'npm install standard --global' to install it" && false)

.PHONY: test-js-style
test-js-style: check-standard
	standard

.PHONY: tidy-js
tidy-js: check-standard
	standard --fix
