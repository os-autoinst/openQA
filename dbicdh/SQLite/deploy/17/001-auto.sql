...
-- Created on Tue Dec  9 11:41:54 2014
-- 
...
--
DROP TABLE IF EXISTS worker_properties;
...
CREATE INDEX job_modules_job_index ON job_modules (job_id);
--
-- Table: job_templates
--
...
DROP TRIGGER IF EXISTS trigger_worker_properties_t_created;
CREATE TRIGGER trigger_worker_properties_t_created after insert on worker_properties BEGIN UPDATE worker_properties SET t_created = datetime('now') WHERE _rowid_ = NEW._rowid_; END;
DROP TRIGGER IF EXISTS trigger_worker_properties_t_updated;
CREATE TRIGGER trigger_worker_properties_t_updated after update on worker_properties BEGIN UPDATE worker_properties SET t_updated = datetime('now') WHERE _rowid_ = NEW._rowid_; END;
...
DROP TRIGGER IF EXISTS trigger_job_modules_t_created;
CREATE TRIGGER trigger_job_modules_t_created after insert on job_modules BEGIN UPDATE job_modules SET t_created = datetime('now') WHERE _rowid_ = NEW._rowid_; END;
DROP TRIGGER IF EXISTS trigger_job_modules_t_updated;
CREATE TRIGGER trigger_job_modules_t_updated after update on job_modules BEGIN UPDATE job_modules SET t_updated = datetime('now') WHERE _rowid_ = NEW._rowid_; END;
