		...
		<table style="width: 95%;">
			...
			<tr class="<?= cycle() ?>">
				...
				<td class="links" style="width: 60%;">
					...
					<a href="/viewimg/<?= $prj ?>/testresults/<?= $testname ?>/<?= $screenshot->{'name'} ?>.png" title="<?= $screenshot->{'name'} ?>.png"><img src="/<?= $prj ?>/testresults/<?= $testname ?>/<?= $screenshot->{'name'} ?>.png?size=<?= $ref_width ?>x<?= $ref_height ?>" width="<?= $ref_width ?>" height="<?= $ref_height ?>" alt="<?= $screenshot->{'name'} ?>.png" class="<?= "resborder\L$screenshot->{'result'}" ?>" /></a>
					<? } ?>
				</td>
