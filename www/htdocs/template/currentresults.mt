	...
	<table style="width: 95%;" id="results" class="table-autosort table-autofilter table-autostripe table-stripeclass:odd">
		...
		<tr>
			<td>
				...
				<a href="/schedule/<?= $test->{'testname'} ?>?action=restart&back=results"><img src="/images/toggle.png" alt="restart" title="Restart Job"/></a>
				<? } ?>
			</td>
			<td><?= $test->{'backend'} ?></td>
			<td><?= $test->{'distri'} ?></td>
...
? #<td colspan="3"><?= $test->{'run_stat'}->{'moddone'} ?> / <?= $test->{'run_stat'}->{'modcount'} ?></td>
		...
		<tr>
			...
			<td colspan="4" style="padding: 3px 4px; font-style: italic;">
				<a href="/schedule/<?= $test->{'testname'} ?>?action=cancel" onclick="this.href += '&back=results'" rel="nofollow">cancel</a>
			</td>
		</tr>
		<? } ?>
		</tbody>
