...
@warnings = warnings { $rsp = $t->post_ok('/api/v1/isos', form => {ISO => $iso, KERNEL_URL => 'http://localhost/vmlinuz', DISTRI => 'opensuse', VERSION => '13.1', FLAVOR => 'DVD', ARCH => 'i586', BUILD => '0091'})->status_is(200) };
map { like($_, $expected, 'expected warning') } @warnings;
