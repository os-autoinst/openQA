...
my $job = OpenQA::Scheduler::Scheduler::job_grab(workerid => $w1_id, scheduler => 1);
is($job->{id}, $jobA->id, "'client' worker should get 'client' job even though 'server' job has higher prio");

$job = OpenQA::Scheduler::Scheduler::job_grab(workerid => $w2_id, scheduler => 1);
is($job->{id}, $jobB->id, "'server' job for 'server' worker");

$job = OpenQA::Scheduler::Scheduler::job_grab(workerid => $w3_id, scheduler => 1);
is($job->{id}, $jobE->id, "32bit worker gets 32bit job with highest prio");

$job = OpenQA::Scheduler::Scheduler::job_grab(workerid => $w4_id, scheduler => 1);
is($job->{id}, $jobF->id, "next job by prio");

$job = OpenQA::Scheduler::Scheduler::job_grab(workerid => $w5_id, scheduler => 1);
is($job->{id}, $jobD->id, "next job by prio, 'client' worker can do jobs without class");

$job = OpenQA::Scheduler::Scheduler::job_grab(workerid => $w6_id, scheduler => 1);
is($job->{id}, $jobC->id, "next job by prio, 64bit worker can get 32bit job");

$job = OpenQA::Scheduler::Scheduler::job_grab(workerid => $w7_id, scheduler => 1);
is($job->{id}, $jobH->id, "next job by prio, parent - server");

$job = OpenQA::Scheduler::Scheduler::job_grab(workerid => $w8_id, scheduler => 1);
is($job->{id}, $jobJ->id,
...
$job = OpenQA::Scheduler::Scheduler::job_grab(workerid => $w9_id, scheduler => 1);
