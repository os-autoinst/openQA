...
subtest 'OpenQA::Git signal handling' => sub {
    ...
    $git_mock->redefine(
        run_cmd_with_log_return_error => sub { {status => 0, stdout => '', stderr => '', signal => 2} });
    throws_ok { $git->commit({add => ['file.txt']}) } qr{OpenQA::Error::Cmd: Unable to add via Git},
      'Aborts git command when signal was received';

    $signal_guard_mock->redefine(signaled => 2);
    throws_ok { $git->commit({add => ['file.txt']}) } qr{OpenQA::Error: Not running git command.*signal 2},
    ...
    subtest 'rollback commit' => sub {
        ...
        my @results = (
            ...
            {status => 0, return_code => 0, stdout => '', stderr => '', signal => 2},
        ...
        subtest 'git commit' => sub {
            ...
            throws_ok { $git->commit({add => ['file.txt']}) } qr{OpenQA::Error::Cmd: Unable to commit},
              'git commit fails';
            for my $i (0 .. $#expected) {
        ...
        subtest 'git push' => sub {
            ...
            @executed = ();
            throws_ok { $git->commit({add => ['file.txt']}) } qr{OpenQA::Error::Cmd: Unable to push}, 'git push failed';
            for my $i (0 .. $#expected) {
                like "@{$executed[$i]}", $expected[$i], "executed git command $i like expected";
            }
        };
