...
subtest 'filter helper links' => sub {
    ...
    $driver->find_element('#filter-form button[type="submit"]')->click();
    wait_until sub { $driver->get_current_url =~ qr/result(__n)?=/ }, 'form submitted after using helper links';
    like $driver->get_current_url, qr/result(__n)?=/, 'URL contains result parameter';
};
...
subtest 'empty query parameters are removed' => sub {
    ...
    # set one value and leave others empty
...
subtest 'redundant parameters are removed' => sub {
    ...
    # select "ok" meta and also manually select "passed" (which is redundant)
    $driver->find_element_by_id('filter-ok')->click();
    my $passed_cb = $driver->find_element_by_id('filter-passed');
    ok $passed_cb->is_selected, 'passed selected by meta-ok';
    # Uncheck and check passed to make it explicitly present if it wasn't
    # (though meta-ok already checks it, we want to ensure the logic handles it)
    $passed_cb->click();
    $passed_cb->click();
...
subtest 'all but one results in negative filter' => sub {
    ...
    wait_until sub { $driver->get_current_url =~ qr/result__n=passed/ }, 'form submitted';
    like $driver->get_current_url, qr/result__n=passed/, 'URL contains result__n=passed';
};
