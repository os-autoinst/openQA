...
subtest 'stacking of parallel children' => sub {
    ...
    $driver->refresh;
    my $toggle_button = wait_for_element(selector => '.toggle-parallel-children', is_displayed => 1);
    ...
    $collapse_all_button = wait_for_element(selector => '.collapse-all-button', is_displayed => 1);
    my $expand_all_button = wait_for_element(selector => '.expand-all-button', is_displayed => 1);
    return unless $collapse_all_button && $expand_all_button;
...
sub check_textmode_test ($test_row) {
    ...
    my @job_rows;
    for my $row (@{$driver->find_elements('#content tbody tr')}) {
        my $text = $row->get_text;
