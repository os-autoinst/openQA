...
my $params = Test::MockObject->new->set_always(pairs => ['openid.op_endpoint', 'https://www.opensuse.org/openid/']);
...
$mock_openid_consumer->redefine(
    handle_server_response => sub ($self, %res_handlers) {
        $res_handlers{setup_needed} ? $res_handlers{setup_needed}->('https://www.opensuse.org/openid/setup') : undef;
    });
$c->set_always(headers => Mojo::Headers->new);
$app->set_always(log => Test::MockObject->new->set_true('debug'));
