...
subtest 'controller->_get_needles_ref_and_url' => sub {
    ...
    subtest 'without fragment and needles_git_hash in vars' => sub {
        ...
        my $needle_refs = $controller->_get_needles_ref_and_url($job);

        is($needle_refs, undef, 'needles refs cannot be defined without vars.json');
    };
    ...
    subtest 'with vars' => sub {
        ...
        my $vars_json_content = '{"NEEDLES_GIT_HASH": "0123456789abcdef0123456789abcdef01234567", "NEEDLES_GIT_URL": "ssh://git@codeberg.org/foo/bar.git"}';
...
subtest 'needledir set by jsonfile_in_temp_dir' => sub {
    ...
    $pngfile->spew('This is obvously a png file');
    $t->get_ok('/needles/' . $settings{DISTRI} . '/needle.png?jsonfile=' . $jsonfile)->status_is(200)->content_like(qr/This is obvously a png file/);
};
...
subtest '_locate_needle_for_ref' => sub {
    ...
    my $temp_dir = path(OpenQA::Needles::temp_dir);
    note `git -C "$needles_dir" init`;
    note `git -C "$git_remote_dir" init`;
    path($git_remote_dir, 'foo.png')->spew('this is a png');
    path($git_remote_dir, 'foo.json')->spew('this is json');
    note `git -C "$git_remote_dir" config user.email foo\@bar`;
    note `git -C "$git_remote_dir" config user.name foo`;
    note `git -C "$git_remote_dir" add foo.png foo.json`;
    note `git -C "$git_remote_dir" commit -m cmsg`;
