...
BAIL_OUT 'invalid SCALABILITY_TEST_WORKER_COUNT' if !looks_like_number($worker_count) || $worker_count <= 0;
...
my @job_counts
  ...
  ? ($ENV{SCALABILITY_TEST_JOB_COUNT})
  : sort { $a <=> $b } grep { $_ > 0 } ($worker_count - 2, $worker_count, $worker_count + 2);
my %seen;
