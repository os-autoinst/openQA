...
subtest 'clickable summary buttons' => sub {
    ...
    like($failed_link->attr('href'), qr/distri=opensuse/, 'preserves distri');
    like($failed_link->attr('href'), qr/version=Factory/, 'preserves version');
    like($failed_link->attr('href'), qr/build=0048/, 'preserves build');
    like($failed_link->attr('href'), qr/result=failed/, 'sets result filter');

    my $softfailed_link = $summary->at('a[href*="result=softfailed"]');
    ok($softfailed_link, 'Soft-failed count is clickable');
    like($softfailed_link->attr('href'), qr/result=softfailed/, 'sets softfailed filter');
