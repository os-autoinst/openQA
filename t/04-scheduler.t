...
subtest 'job grab (no jobs because max_running_jobs is 0)' => sub {
    ...
    my @jobs;
    push @jobs, $jobs->create_from_settings(\%settings2) for 1 .. 10;
    my $scheduler_conf = OpenQA::App->singleton->config->{scheduler};
    local $scheduler_conf->{max_running_jobs} = 0;
...
subtest 'scheduler limits' => sub {
    ...
    subtest 'job grab (no jobs because max_running_jobs limit is exceeded)' => sub {
        ...
        my $scheduler_conf = OpenQA::App->singleton->config->{scheduler};
        local $scheduler_conf->{max_running_jobs} = 2;
        my $res = OpenQA::Scheduler::Model::Jobs->singleton->schedule();
...
subtest 'dynamic job limit' => sub {
    my $scheduler = OpenQA::Scheduler::Model::Jobs->singleton;
    my $conf = OpenQA::App->singleton->config->{scheduler};

    subtest 'dynamic limit disabled: returns static max_running_jobs' => sub {
        local $conf->{dynamic_job_limit_enabled} = 0;
        local $conf->{max_running_jobs} = 42;
    ...
    subtest 'dynamic limit enabled with low load: returns current dynamic limit' => sub {
        ...
        local $conf->{dynamic_job_limit_load_threshold} = 8;
        local $conf->{dynamic_job_limit_load_critical} = 16;
    ...
    subtest 'dynamic limit enforced in scheduling: fewer jobs than max_running_jobs allocated' => sub {
        ...
        local $conf->{dynamic_job_limit_load_threshold} = 8;
        local $conf->{dynamic_job_limit_load_critical} = 16;
        local $conf->{dynamic_job_limit_step} = 1;
        local $conf->{dynamic_job_limit_interval} = 0;
        $scheduler->dynamic_limit(OpenQA::Scheduler::DynamicLimit->new);
