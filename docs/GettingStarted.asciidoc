...
A job goes through following states:

* *scheduled* Initial state for newly created jobs. The scheduler suppose to assign
  the job to a worker.
* *assigned* The job has been sent to worker but worker has not acknowledged yet.
   the state might be reverted to *scheduled* in some conditions
* *setup* The worker prepares execution of backend/isotovideo
   (e.g. waiting for cache service)
* *running* backend/isotovideo is running
* *uploading* Remaining test results are uploaded after backend/isotovideo
  has exited
* *cancelled* job was cancelled by the user (result *user_cancelled* is set)
   or obsoleted due to a new build (result *obsoleted* is set)
   or skipped due to failed (directly) chained dependencies (result *skipped* is set).
* *done*  worker reported that the job is no longer running (a result other than NONE but including
...
and in the
https://github.com/os-autoinst/openQA/tree/master/etc/openqa[Git repository under `etc/openqa`].
...
https://en.opensuse.org/openSUSE:Packaging_UsrEtc#Variant_1_(ideal_case)[package maintainer]
...
<<Contributing.asciidoc#_customize_configuration_directory,customizing the configuration directory>>.
...
of <<GettingStarted.asciidoc#_configuration,the web UI configuration>>. Jobs outside
