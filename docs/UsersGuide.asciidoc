...
=== Finding tests by quering particular job settings

In case you are looking for a way to get a list of all the tests based on their
settings, here two ways to achieve it.

[source,sh]
----
curl -s "https://openqa.suse.de/api/v1/job_settings/jobs?key=*_TEST_ISSUES&list_value=39911"
----

or with the power of the `openqa-cli api`
...
It is possible to use a literal string or a glob for C<key> as is shown in the
examples. We cannot use glob for the C<list_value>.
OpenQA configuration sets C<job_settings_max_recent_jobs> value which limits
the results.
...
* Archiving of important jobs can be enabled. Checkout the related settings
