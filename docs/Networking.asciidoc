...
NOTE: The number of tap devices should correspond to the number of the
running workers. For example, if you have set up 3 tap devices, the same number of
workers should be configured. 
...
NOTE: It is also possible to switch using YaST.
...
When everything is ok and the machines are able to communicate, the
ovs-vsctl should show something like the following
...
NOTE: Notice the tag numbers are assigned to tap1 and tap2. They should have the same number.

NOTE: If the balance of the tap devices is wrong in the workers.ini the tag cannot be assigned and the communication will be broken.
