...
`ping_server`/`ping_client` or `wicked_basic_ref`/`wicked_basic_sut`. To test
GRE tunnels, you may want to change the jobs worker classes so the different
jobs are executed on different workers.
