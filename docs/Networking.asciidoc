...
[NOTE]
In some cases (e.g. on Leap) can be needed to start the OpenvSwitch service before the Network service by modifying the OpenvSwitch service file /usr/lib/systemd/system/openvswitch.service, where create an entry `Before=network.service` and remove network.service from `After=network.service`. For reference see https://en.opensuse.org/Portal:Wicked/OpenvSwitch#Wicked_0.6.23.2B[this].
