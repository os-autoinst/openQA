...
== External WebAPI Plugins

During processing of `openqa.ini` all sections with prefix "PluginWebAPI" are examined 
...
Parameters from the configuration file section will be passed to plugin's `init()` method.
...
[PluginWebAPIFoo]
enabled=optional
path=/etc/bar/OpenQA/Plugin/WebAPI
Msg=Test
----
The service will attempt to load plugin: `/etc/bar/OpenQA/Plugin/WebAPI/Foo.pm`.
`Msg` parameter will be passed to plugin `register()` method.

At the moment following restrictions apply:
* plugin routes will be mapped to `/plugin` and require at least operator access level.
* to be able being loaded into the service, module must implement 'Mojolicious::Plugin' and 
belong to `OpenQA::Plugin::WebAPI` namespace. Side effect requirement is that `path` 
parameter in configuration file must end with `OpenQA/Plugin/WebAPI`.
* application will fail to start on plugin load error in case if `enabled` parameter is set 
to `required`.

See also example plugin within `t/data/OpenQA/Plugin/WebAPI`.
