...
Example commands to manually fix ci-packages.txt:

```
# must be executed from root source folder
docker run --rm -v$(pwd):/opt/testing_area registry.opensuse.org/devel/openqa/ci/containers/base:latest .circleci/build_dependencies.sh
```

Then review `git diff .circleci/ci-packages.txt` and commit the change.
