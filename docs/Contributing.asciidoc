...
- dependencies.txt lists dependencies to test against.
...
One way is to build an image using `build_local_docker.sh` script, start a
...
Create an image called `localtest` based on the content of `dependnencies.txt`
and `autoinst`:
```
...
After installing the `circleci` tool following commands will be available.
...
Command to verify yaml with the `circleci` tool:
```
