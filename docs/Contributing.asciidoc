...
NOTE: `run-tests-within-container` runs with CONTAINER_TEST enabled by default. It is used to run scripts which are not meant to run inside the *openqa_devel* and the image itself does not contain any container engine. Consider disabling it to run any tests in a container.
