...
** Let http://perltidy.sourceforge.net/[Perltidy] tidy perl files
automatically:

    tools/tidyall --git # tidy changed files
    tools/tidyall --all # tidy all files
    make tidy # to tidy also JavaScript files in the openQA project
...
In the case of os-autoinst, some http://www.cpan.org/[CPAN] modules are
required. Additionally, several external tools are needed including
...
For openSUSE there is `openQA` and `os-autoinst`, including several subpackages.
That should be everything necessary to run an openQA instance.

```
zypper install openQA os-autoinst
```
...
There is a <<Installing.asciidoc#single-instance-container,Single-Instance Container>>
which can be tried out as a minimal installation.
...
For openSUSE:
```
zypper install os-autoinst-devel openQA-devel
```
...
For something to mostly just copy & paste, check out the
<<Contributing.asciidoc#quick-container-development-setup,Quick container
development setup>>.

Otherwise follow the detailed steps below. This setup leaves it up to
you if you run this in a container or not.
...
Time to grab a hot beverage while installing…
...
For playing with the web UI, you just need to run this one daemon.
