...
e.g. to use postgresql or mysql you have to change etc/openqa/database.ini 
or to increase the log level it's useful to set the loglevel = debug in
...
--------------------------------------------------------------------------------
mkdir etc/mine
cp etc/openqa/database.ini etc/mine
cp etc/openqa/openqa.ini etc/mine
cp etc/openqa/client.conf etc/mine
cp etc/openqa/workers.ini etc/mine
export OPENQA_CONFIG=$PWD/etc/mine
--------------------------------------------------------------------------------

Note that OPENQA_CONFIG points to the directory in which openqa.ini, database.ini,
client.conf and workers.ini are expected to reside.
