...
At WebUI service startup during configration file processing all sections starting with 
"WebAPIPlugin" are examined for parameters `enabled` and `path`. 
(Tail of section name is considered as name of plugin.)
Unless parameter `enabled` is missing or set to `0` - the process will attempt to load from 
`WebAPIPlugin` folder within path configured:
- json file with OpenAPI format describing routes udner 'plugin';
- perl module.
Parameters from the configration file secion will be passed to plugin init() method.

E.g. with configration file section like below:
[source]
----
[WebAPIPluginFoo]
enabled=optional
...
- no app data is shared between main app and plugin;
...
to `required`;
- to be able being loaded into service, plugin's perl module must implement 'Mojolicious::Plugin';
- to be able serve routes properly plugin's perl module must be within WebAPIPlugin namespace and
