...
=== Javascript and CSS
Add third-party JavaScript and CSS file to `assets/assetpack.def`. When
restarting the web server the new/updated files are downloaded automatically.

Make sure to add those files to Git when committing your changes. Additionally,
add the license(s) for the newly added third-party code to the root directory of
the repository. Do *not* duplicate common/existing licenses; extend the
`Files:`-section at the beginning of those files instead.
