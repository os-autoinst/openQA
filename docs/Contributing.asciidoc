...
Then you can launch the tests:
  ...
  docker run -v OPENQA_LOCAL_CODE:/opt/openqa -v /var/run/dbus:/var/run/dbus -e VAR1=1 -e VAR2=1 registry.opensuse.org/devel/openqa/containers/openqa_dev make docker-tests
...
It is also possible to use a custom os-autoinst checkout using the following arguments:
  ...
  docker run ... -e CUSTOM_OS_AUTOINST=1 -v OS_AUTOINST_LOCAL_CODE:/opt/os-autoinst make docker-tests

Note that in this case your checkout must be ready to use so +configure+ and +make+ need to be executed before.
