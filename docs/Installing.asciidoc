...
operations. By default openQA is configured to:

* Automatically update the repository when scheduling tests
* Clone tests and needles from git repos specified in a job's `CASEDIR` and
  `NEEDLES_DIR` variables

To disable the support for example to prevent openQA to do
any operations on a manually maintained working copy the git support can be
completely disabled with
...
Once you do so and restart the web interface, all of the above features will
be disabled.

To enable or disable individual features, you can use these config settings,
e.g. to disable:
...
tests. `git_auto_clone` controls the automatic cloning of repos referenced by
`CASEDIR` and `NEEDLES_DIR`.

To enable automatic commits when needles are created or edited in the webUI
editor set `git_auto_commit`:
