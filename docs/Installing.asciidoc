...
https://www.terraform.io/[Terraform] provides the means to deploy in a
cloud environment via recipes. In order to use this with AWS a file
`terraform.tfvars` needs to be prepared with the following values:
...
A suitable image can be found on
https://pint.suse.com/?resource=images&csp=amazon&region=eu-central-1&search=leap[PINT]
. For additional settings please refer to `container/terraform/variables.tf`.
