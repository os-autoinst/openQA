...
To setup openQA you will need a PostgreSQL database available to the user
running openQA. You need to create a database and a dedicated user account with
full access to it. To configure access to the chosen database in openQA,
edit +/etc/openqa/database.ini+ and change the settings in the +[production]+ section.

The +dsn+ value format technically depends on the database type and is
documented for PostgreSQL at {DBD-Pg-DBI_Class_Methods}
