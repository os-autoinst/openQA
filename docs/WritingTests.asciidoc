...
The openQA webUI scheduler also supports a dynamic global job limit that scales the effective
number of allowed running jobs up or down based on the system load of the openQA webUI host.
...
The running jobs heading in the webUI reflects the current dynamic limit when it is active.
