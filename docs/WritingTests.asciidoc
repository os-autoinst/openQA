...
* +CHAINED+ and +DIRECTLY_CHAINED+ describe when one test case depends on another and both are run sequentially, i.e. KDE test suite is run after and only after Installation test suite
...
To define a +CHAINED+ dependency add the variable +START_AFTER_TEST+ with the name(s) of test suite(s) after which selected test suite is supposed to run.
Use comma separated list for multiple test suite dependency. E.g. +START_AFTER_TEST="kde,dhcp-server"+

To define a +DIRECTLY_CHAINED+ add the variable +START_DIRECTLY_AFTER_TEST+. It works in the same way as for +CHAINED+ dependencies. Make sure all tests have the same worker class.

* +PARALLEL+ describes mulit-machine tests. That are test suites scheduled to run at the same time and managed as a group. On top of that, +PARALLEL+ also describes
