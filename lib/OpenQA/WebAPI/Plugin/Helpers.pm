...
sub register ($self, $app, $config) {
    ...
    $app->helper(
        bugtitle_for => sub ($c, $bugid, $bug = undef) {
            my $text = "Bug referenced: $bugid";
            $text .= "\n" . $bug->title if $bug && $bug->existing && $bug->title;
            return $text;
        });

    $app->helper(bug_report_actions => sub ($c, %args) { $c->include_branding('external_reporting', %args) });
    $app->helper(human_readable_size => sub ($c, $size) { human_readable_size($size) });
    ...
    $app->helper(
        steploglink_for => sub ($c, $title, $url, $icon, $class = '') {
            my $icons = $c->t(i => (class => "step_action fa-regular fa-lg fa-stack-1x $icon"));
            my $content = $c->t(span => (class => 'fa-stack') => sub { $icons });
            return $c->link_to($url => (title => $title, class => $class) => sub { $content });
        });
    ...
    $app->helper(is_operator_js => sub ($c) { Mojo::ByteStream->new($c->helpers->is_operator ? 'true' : 'false') });
    $app->helper(is_admin_js => sub ($c) { Mojo::ByteStream->new($c->helpers->is_admin ? 'true' : 'false') });
    ...
    $app->helper(
        ...
        help_popover =>
          sub ($c, $title, $content, $details_url = undef, $details_text = undef, $placement = undef, @args) {
            my $class = 'help_popover fa-solid fa-circle-question';
            if ($details_url) {
                $content
                  .= '<p>See '
    ...
    $app->helper(
        group_link_menu_entry => sub ($c, $group) {
            return $c->tag(
                'li',
                $c->link_to(
    ...
    $app->helper(
        comment_icon => sub ($c, $jobid, $comment_count = undef) {
    ...
    $app->helper(
        setting_link => sub ($c, $uri, $jobid = undef) {
    ...
    $app->helper(
        'reply.gru_result' => sub ($c, $result, $error_code = 200) {
            return $c->render(json => $result, status => $error_code);
        });
    ...
    $app->helper(link_key_exists => sub ($c, $value) { exists $c->app->config->{settings_ui_links}->{$value} });
...
sub _validation_error ($c, $args = {}) {
    my $format = $args->{format} // 'text';
    my @errors;
    for my $parameter (@{$c->validation->failed}) {
        push @errors, exists($c->validation->input->{$parameter}) ? "$parameter invalid" : "$parameter missing";
    }
    my $error = 'Erroneous parameters (' . join(', ', @errors) . ')';
