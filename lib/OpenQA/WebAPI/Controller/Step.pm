...
- Managing Git-based needle storage and retrieval.
...
sub _basic_needle_info ($self, $name, $distri, $version, $file_name, $needles_dir, $needle_ref = undef, $needle_url = undef) {
    $file_name //= "$name.json";
    $file_name = locate_needle($file_name, $needles_dir, $needle_ref, $needle_url) if !-f $file_name;
...
sub _extended_needle_info ($self, $needle_dir, $needle_name, $basic_needle_data, $file_name, $error, $error_messages,
    $needle_ref = undef, $needle_url = undef)
{
...
sub viewimg ($self) {
    ...
    if (my $needle = $module_detail->{needle}) {
        my ($needleinfo)
          = $self->_basic_needle_info($needle, $distri, $dversion, $module_detail->{json}, $needle_dir, $needle_ref, $needle_url);
        if ($needleinfo) {
            my $info = {
                name => $needle,
    ...
    if ($module_detail->{needles}) {
        for my $needle (@{$module_detail->{needles}}) {
            ...
            my ($needleinfo)
              = $self->_basic_needle_info($needlename, $distri, $dversion, $needle->{json}, $needle_dir, $needle_ref, $needle_url);
