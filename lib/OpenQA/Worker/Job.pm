...
sub _upload_asset {
    ...
    $ua->upload->on(
        'upload_chunk.finish' => sub ($upload, $piece, @args) {
    ...
    my $response_cb = sub ($upload, $tx, @args) {
    ...
    $ua->upload->on(
        'upload_chunk.fail' => sub ($upload, $res, $chunk, @args) {
