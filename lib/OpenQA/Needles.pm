...
sub _locate_needle_for_ref ($relative_needle_path, $needles_dir, $needles_ref, $needle_url) {
    ...
    my $allow_arbitrary_url_fetch = OpenQA::App->singleton->config->{'scm git'}->{allow_arbitrary_url_fetch} eq 'yes';
    ...
    my $error = $git->cache_ref($needles_ref, $needle_url, $relative_needle_path, $temp_json_path, $allow_arbitrary_url_fetch)
