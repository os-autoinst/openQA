...
sub _assign_allocated_job_worker_pairs {
    ...
    log_debug "Worker already got jobs, skipping" and return if $worker->unfinished_jobs->count;
    ...
    my $cluster_info   = $self->scheduled_jobs->{$first_job_id}->{cluster_jobs};
    my $jobs_resultset = $schema->resultset('Jobs');
    my %sort_criteria  = map {
        my $job_id = $_;
        my $sort_criteria;
        if (my $scheduled_job = $self->scheduled_jobs->{$job_id}) {
    ...
    try {
        @jobs = $jobs_resultset->search({id => {-in => $job_ids}});
    }
    catch {
    ...
    try {
        if ($actual_job_count > 1) {
            ...
            $res = $self->_assign_multiple_jobs_to_worker(\@jobs, $worker, $directly_chained_job_sequence, $job_ids);
        }
        else {
            $schema->txn_do(
                sub {
    ...
    if (ref($res) eq 'HASH' && $res->{state} && $res->{state}->{msg_sent} == 1) {
        ...
        return map { {job => $_, worker => $worker_id} } @$job_ids;
    }
