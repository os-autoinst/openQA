...
sub _run_cmd ($self, $args, $options = {}) {
    ...
    my $result = run_cmd_with_log_return_error(\@cmd, expected_return_codes => $options->{expected_return_codes}, output_file => $options->{output_file});
...
sub cache_ref ($self, $ref, $url, $relative_path, $output_file, $allow_arbitrary_url_fetch = 1) {
    OpenQA::App->singleton->log->debug("Checking out $relative_path from url '" . ($url // '') . "' rev '" . ($ref // '') . "' to $output_file");
    ...
    my $res = run_cmd_with_log_return_error [@git, 'cat-file', '-t', $ref];
    ...
    # create output dir
    path(dirname($output_file))->make_path;
