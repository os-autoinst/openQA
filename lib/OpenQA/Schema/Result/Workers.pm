...
sub reserve ($self, $user, $comment = undef, $duration = undef, $force = 0, $worker_class = undef, $scope = 'instance')
{
    ...
    die reservation_error(invalid => "Invalid specific worker class '$worker_class'")
      if defined $worker_class && !reservation_class_valid($worker_class);
...
sub accepts_classes ($self, $needed, $reservation_class = undef) {
    return 0 if defined $reservation_class && !any { $_ eq $reservation_class } @$needed;
    return !any { (!defined $reservation_class || $_ ne $reservation_class) && !$self->check_class($_) } @$needed;
}
...
sub info ($self) {
    ...
    $settings->{properties} = {%properties{grep { !/^RESERVED_/ } keys %properties}};
