...
function previewSuccess(stepPreviewContainer, data, force) {
  ...
  const linksTd = stepPreviewContainer.closest('td');
  if (linksTd.previousElementSibling.getAttribute('mode') === 'log') {
    const caretSpace = 15; // .fa-caret-up is positioned bottom: -25px
    pout.style.top = stepPreviewContainer.offsetTop + stepPreviewContainer.offsetHeight + caretSpace + 'px';
  } else {
    pout.style.top = '';
  }
  ...
  if (!(pin.querySelector('pre') || pin.querySelector('audio'))) {
    ...
    setDiffScreenshot(imageSource, complete);
  } else {
    complete();
  }
...
function setCurrentPreview(stepPreviewContainer, force) {
  if (stepPreviewContainer?.jquery) stepPreviewContainer = stepPreviewContainer[0];
  ...
  if (textResultElement) {
    ...
    hidePreviewContainer();
    setPageHashAccordingToCurrentTab(textResultElement.dataset.href);
  ...
  if (link.dataset.text) {
    stepPreviewContainer.classList.add('current_preview');
    setPageHashAccordingToCurrentTab(link.getAttribute('href'));
    const text = unescape(link.dataset.text);
    const linksTd = stepPreviewContainer.closest('td');
    if (linksTd.previousElementSibling.getAttribute('mode') === 'log') {
      hidePreviewContainer();
      const log_container = stepPreviewContainer.nextElementSibling;
      if (log_container) {
        const div = log_container.querySelector('div');
...
function filterEmbeddedLogFiles() {
  ...
  if (searchBox) {
    searchBox.value = parseQueryParams().filter?.[0] ?? searchBox.value;
  }
...
function setCurrentPreviewFromStepLinkIfPossible(stepLink) {
  if (!stepLink) return;
  const parent = stepLink.parentElement;
  if (tabConfiguration.details.hasContents && parent && !parent.classList.contains('current_preview')) {
    setCurrentPreview(parent);
  }
}
...
function setupTestDetailsFilter(tabConfig) {
  ...
  const applyFilterChanges = function (event) {
    ...
    if (anyFilterEnabled) {
      nameFilter = detailsNameFilter?.value ?? '';
      nameFilterEnabled = nameFilter.length !== 0;
      failedOnlyFilterEnabled = detailsFailedOnlyFilter?.checked ?? false;
  ...
  detailsNameFilter?.addEventListener('keyup', applyFilterChanges);
  detailsFailedOnlyFilter?.addEventListener('change', applyFilterChanges);
