...
function previewSuccess(stepPreviewContainer, data, force) {
  ...
  const currentTd = stepPreviewContainer.closest('td').get(0);
  if (currentTd.previousElementSibling.getAttribute('mode') === 'log') {
...
function showTextBoxes(el) {
  ...
  divs.forEach(div => {
    ...
    if (currentTd.getAttribute('mode') === 'log') {
  ...
  if (currentTd.getAttribute('mode') === 'log') {
    currentTd.setAttribute('mode', 'default');
    ...
    currentTd.setAttribute('mode', 'log');
