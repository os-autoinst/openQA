function setupJobNextPrevious () {
  ...
  const setPage = function (json) {
    ...
    const current_index = json.data.map(function (n) { return n.iscurrent }).indexOf(1)
...
function renderMarks (data, type, row) {
  let html = '<span class="badge badge-info float-right" title="'
...
function renderJobResults (data, type, row) {
  ...
  for (const i in row.failedmodules) {
    ...
    const async_url = '/tests/' + row.id + '/modules/' + htmlEscape(row.failedmodules[i]) + '/fails'
...
function renderBuild (data, type, row) {
  const link = '/tests/overview?distri=' + row.distri + '&version=' + row.version + '&build=' + row.build
