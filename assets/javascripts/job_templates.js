...
function makePrioCell(prio, disabled) {
  ...
  const defaultPrioEl = document.getElementById('editor-default-priority');
  const defaultPrio = defaultPrioEl ? defaultPrioEl.dataset.initialValue : null;
  if (!defaultPrio) {
    defaultPrio = 50;
  }
  if (!prio) {
...
function buildMediumGroup(group, media) {
  ...
  prioHelpPopover.className = 'help_popover fa-solid fa-circle-question';
