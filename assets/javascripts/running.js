...
function addDataListener(elem, callback) {
  ...
  if (!elem.eventCallback) {
    elem.eventCallback = function (event) {
      ...
      const currentData = elem.innerHTML;
...
function updateDeveloperPanel() {
  ...
  if (developerMode.develSessionDeveloper) {
    ...
    const globalSessionInfoElement = document.getElementById('developer-global-session-info');
    if (
...
function handlePauseOnMismatchSelected() {
  ...
  const selectedValue = document.getElementById('developer-pause-on-mismatch').value;
  let pauseOn;
...
||||||| parent of dc1b68471 (refactor(ui): remove jQuery from main application scripts)
          $.each(messageToStatusVariable, function (index, msgToStatusValue) {
            var msg = msgToStatusValue.msg;
=======
          messageToStatusVariable.forEach(msgToStatusValue => {
            var msg = msgToStatusValue.msg;
>>>>>>> dc1b68471 (refactor(ui): remove jQuery from main application scripts)
            if (!(msg in data)) {
              return;
            }
