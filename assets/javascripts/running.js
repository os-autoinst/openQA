...
function sendCommand (command) {
  const wid = testStatus.workerid
  if (wid == null) return false
  const url = $('#canholder').data('url').replace('WORKERID', wid)
  $.ajax({
    url: url,
    type: 'POST',
...
function addDataListener (elem, callback) {
  ...
  if (!elem.eventCallback) {
    elem.eventCallback = function (event) {
      ...
      const newData = JSON.parse(event.data)[0]
...
function updateDeveloperPanel () {
  ...
  developerModeElements.each(function (index) {
    ...
    const hide = ((hiddenOn && developerMode.prop(hiddenOn)) ||
  ...
  const modules = moduleToPauseAtOptions.map(function () {
    const option = $(this)
  ...
  const moduleToPauseAtStillAhead = (developerMode.moduleToPauseAt &&
  ...
  if (developerMode.develSessionDeveloper) {
    ...
    const timeagoElement = $('<abbr class="timeago" title="' + developerMode.develSessionStartedAt + ' Z">' + developerMode.develSessionStartedAt + '</abbr>')
    sessionInfoElement.append(timeagoElement)
    timeagoElement.timeago()

    const tabsOpenInfo = ', developer has ' + developerMode.develSessionTabCount + (developerMode.develSessionTabCount == 1 ? ' tab' : ' tabs') + ' open)'
