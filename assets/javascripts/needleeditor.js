...
function loadTagsAndName() {
  ...
  const needle = window.needles[tagsSelect.value];
...
function loadAreas() {
  ...
  const needle = window.needles[areaSelect.value];
...
function saveNeedle(overwrite) {
  ...
  const errors = [];
  const tagsSelect = document.getElementById('tags_select');
  const tagSelection = window.needles[tagsSelect ? tagsSelect.value : null];
  if (tagSelection && !tagSelection.tags.length) {
    errors.push('No tags specified.');
  }
  const areaSelect = document.getElementById('area_select');
  const areaSelection = window.needles[areaSelect ? areaSelect.value : null];
  const takeMatches = document.getElementById('take_matches');
  const takeMatchesChecked = takeMatches && takeMatches.checked;
  if (
    areaSelection &&
