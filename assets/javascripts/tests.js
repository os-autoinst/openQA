...
function renderTestName(data, type, row) {
  ...
  if (is_operator) {
    ...
    if (row.result !== 'none') {
      ...
      html += '<i class="action fa-solid fa-fw fa-xmark-circle-o" title="Cancel job"></i></a>';
    }
  }
...
function renderTestResult(data, type, row) {
  ...
  if (row.deps.has_parents) {
    dependencyResultHtml = row.deps.parents_ok
      ...
      : " <i class='fa-solid fa-unlink' title='dependency failed'></i>";
  }
...
function setupTestButtons() {
  ...
  $(document).on('click', '.cancel', function (event) {
    ...
    const i = $(this).find('i').removeClass('fa-xmark-circle');
    $(this).replaceWith(i);
  });
}
