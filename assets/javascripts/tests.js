...
function renderMediumName (data, type, row) {
  let link = '/tests/overview?build=' + row.build + '&distri=' + row.distri + '&version=' + row.version
  if (row.group) {
    link += '&groupid=' + row.group
  }

  let name = "<a href='" + link + "'>" + 'Build' + row.build + '</a>'
  name += ' of '
...
function renderProgress (data, type, row) {
  ...
  const progressBar = '<div class="' + progressClass + '" role="progressbar" style="width: ' + progressWidth +
...
function renderPriority (data, type, row) {
  ...
  const jobId = row.id
  const decreasePrioLink = ('<a class="prio-down" data-method="post" href="javascript:void(0);" onclick="decreaseJobPrio(' +
        jobId + ', this); return false;"><i class="far fa-minus-square"></i></a>')
  const increasePrioLink = ('<a class="prio-up" data-method="post" href="javascript:void(0);" onclick="increaseJobPrio(' +
...
function renderTestSummary (data) {
  ...
  if (data.softfailed) { html += ' ' + data.softfailed + "<i class='fa module_softfailed fa-star-half' title='modules with warnings'></i>" }
  ...
  if (data.skipped) { html += ' ' + data.skipped + "<i class='fa module_skipped fa-angle-double-right' title='modules externally skipped'></i>" }
  return html
}
...
function renderTestLists () {
  ...
  const runningTable = $('#running').DataTable({
  ...
  const scheduledTable = $('#scheduled').DataTable({
    order: [], // no initial resorting
    ajax: {
  ...
  $(document).on('click', '.restart', function (event) {
    ...
    $.post(restartLink.href).done(function (data, res, xhr) {
      ...
      $.each(urls, function (key, value) {
        ...
        const td = $('#job_' + key).closest('tr').children('td.test')
        const restart_link = td.children('a.restart')
        const i = restart_link.find('i').removeClass('fa-redo')
...
function setupLazyLoadingFailedSteps () {
  ...
  $('.failedmodule').on('show.bs.tooltip', function () {
    ...
    $.getJSON(failedModuleElement.data('async'), function (fails) {
      // adjust href
      const newHref = failedModuleElement.attr('href').replace(/\/1$/, '/' + fails.first_failed_step)
      ...
      let newTitle = '<p>Failed needles:</p><ul>'
