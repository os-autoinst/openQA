  ...
  window.deleteUser = function (id) {
    ...
    $.ajax({
      ...
      error: function (xhr, ajaxOptions, thrownError) {
        if (xhr.responseJSON && xhr.responseJSON.error) { addFlash('danger', xhr.responseJSON.error); } else { addFlash('danger', 'An error has ocurred. Maybe there are unsatisfied foreign key restrictions in the DB for this user.'); }
      }
    });
  };
}
