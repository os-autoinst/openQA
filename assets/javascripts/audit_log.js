    ...
    columnDefs: [
      { targets: 0, visible: false }, {
        ...
        render: function (data, type, row) {
          ...
          { return '<a href="' + audit_url + '?eventid=' + row.id + '" title=' + data + '>' + jQuery.timeago(data + ' UTC') + '</a>' } else { return data }
        }
      },
      { targets: 3, visible: false }, {
        ...
        render: function (data, type, row) {
          if (type === 'display') {
            ...
            try {
              const url = getURLForType(row.event, JSON.parse(row.event_data))
              if (url) {
