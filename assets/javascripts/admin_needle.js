function setupAdminNeedles() {
  function ajaxUrl() {
    ...
    if (!needlesEl) return '';
    const url = needlesEl.dataset.ajaxUrl;
    let lastMatch = document.getElementById('last_match_filter').value;
    let lastSeen = document.getElementById('last_seen_filter').value;
    if (lastMatch === 'custom') {
      lastMatch =
  ...
  const table = new DataTable('#needles', {
    ajax: ajaxUrl(),
    deferRender: true,
    ...
    columnDefs: [
      {
        ...
        render: function (data, type, row) {
          if (type === 'display' && data != 'never') {
            ...
            return "<a href='" + row[ri] + "' title='" + data + "Z'>" + timeago.format(new Date(data)) + '</a>';
          } else {
            return data;
          }
        }
  ...
  function startDeletion(ids) {
    ...
    // hide/show elements
    const deletionQuestion = document.getElementById('deletion-question');
    if (deletionQuestion) deletionQuestion.style.display = 'none';
    const deletionOngoing = document.getElementById('deletion-ongoing');
    if (deletionOngoing) deletionOngoing.style.display = 'block';
    const reallyDelete = document.getElementById('really_delete');
    if (reallyDelete) reallyDelete.style.display = 'none';
    const closeDelete = document.getElementById('close_delete');
    if (closeDelete) closeDelete.style.display = 'none';
    const xDelete = document.getElementById('x_delete');
    if (xDelete) xDelete.style.display = 'none';
    const abortDelete = document.getElementById('abort_delete');
    if (abortDelete) abortDelete.style.display = 'inline-block';
    ...
    const deleteBunchOfNeedles = function () {
      ...
      if (outstandingList?.dataset?.aborted === 'true' || ids.length <= 0) {
        ...
        if (abortDelete) abortDelete.style.display = 'none';
        const deletionFinished = document.getElementById('deletion-finished');
        if (deletionFinished) deletionFinished.style.display = 'block';
        const closeDelete = document.getElementById('close_delete');
        if (closeDelete) closeDelete.style.display = 'inline-block';
        const xDelete = document.getElementById('x_delete');
        if (xDelete) xDelete.style.display = 'inline-block';
